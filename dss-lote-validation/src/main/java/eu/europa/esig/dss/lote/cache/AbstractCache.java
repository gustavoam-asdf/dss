 ...
 */
package eu.europa.esig.dss.lote.cache;

import eu.europa.esig.dss.lote.cache.state.CacheStateEnum;
import eu.europa.esig.dss.lote.cache.state.CachedEntry;
import eu.europa.esig.dss.lote.cache.state.CachedExceptionWrapper;
...
public abstract class AbstractCache<R extends CachedResult> {
	...
	public String dump() {
		...
		if (Utils.isMapEmpty(cachedEntriesMap)) {
			...
			sb.append(")\n");
			for (Map.Entry<CacheKey, CachedEntry<R>> mapEntry : cachedEntriesMap.entrySet()) {
