 ...
 *
 */
public class ISO180135EAAIdentifierListValidator implements EAAStatusValidator {
    ...
    @Override
    public EAAStatusToken validate(EAA eaa, byte[] identifierListDocument) {
        if (!isSupported(eaa)) {
            ...
            if (validatorOptions.hasNext()) {
                for (IdentifierListValidatorFactory factory : loader) {
                    if (factory.isSupported(identifierListDocument)) {
                        ...
                        EAAStatusToken statusToken = identifierListValidator.getStatusToken(identifierBytes);
