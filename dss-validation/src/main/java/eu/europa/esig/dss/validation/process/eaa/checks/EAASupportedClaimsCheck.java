...
public class EAASupportedClaimsCheck extends AbstractMultiValuesCheckItem<XmlValidationProcessEAA> {
    ...
    protected boolean process() {
        List<String> claimNames = eaa.getAllEAAPayloadClaims()
                .stream()
                .map(ClaimWrapper::getName)
    ...
    protected String buildAdditionalInfo() {
        List<String> unsupportedClaims = eaa.getAllEAAPayloadClaims()
                .stream()
                .map(ClaimWrapper::getName)
                .filter(c -> !processValueCheck(c))
                .collect(Collectors.toList());

        return i18nProvider.getMessage(MessageTag.EAA_UNSUPPORTED_CLAIMS, String.join(", ", unsupportedClaims));
    }
