 ...
 * 
 */
public class LoTEJWSCompactValidationTask implements Supplier<ValidationResult> {
     ...
     */
    public LoTEJWSCompactValidationTask(DSSDocument loteDocument, CertificateSource signingCertificateSource) {
    ...
    private ValidationPolicy getValidationPolicy() {
        try {
            return ValidationPolicyLoader.fromValidationPolicy(
                    LoTEJWSCompactValidationTask.class.getResourceAsStream(LoTE_VALIDATION_POLICY_LOCATION)).create();
        } catch (Exception e) {
