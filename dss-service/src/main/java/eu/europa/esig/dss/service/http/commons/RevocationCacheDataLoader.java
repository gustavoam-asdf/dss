...
/**
 * This class provides some caching features for revocation data. The default
 * cache folder is set to {@code java.io.tmpdir}.
 * The urls of the resources is transformed to the
 * file name by replacing the special characters by {@code _}
 */
public class RevocationCacheDataLoader implements DataLoader, DSSCacheFileLoader {

	private static final long serialVersionUID = 1L;

	private static final Logger LOG = LoggerFactory.getLogger(RevocationCacheDataLoader.class);
	 ...
	 */
	private long defaultCacheExpirationTime = 600_000L;
	 ...
	 * Empty constructor
	 */
	public RevocationCacheDataLoader() {
		// empty
	}
	 ...
	 */
	public RevocationCacheDataLoader(DataLoader dataLoader) {
	...
	/**
	 * This method allows to set the file cache directory. If the cache folder does
	 ...
	 *
	 * Default: {@code 600_000L} (10 minutes)
	 *
	 * @param cacheExpirationTimeInMilliseconds value in milliseconds
	 */
	public void setDefaultCacheExpirationTime(long cacheExpirationTimeInMilliseconds) {
		this.defaultCacheExpirationTime = cacheExpirationTimeInMilliseconds;
	}
	 ...
	 * method has been invoked then only the
	...
	/**
	 * This methods allows to indicate which resources must be ignored. It is useful
	 * in a test environment where some of
	...
	/**
	 * This method allows to download a {@code DSSDocument} from a specified
	 * {@code url} with a custom setting
	 * indicating whether the {@code refresh} of the document's cache shall be
	 * enforced, when applicable
	 *
	 * @param url     {@link String} remote location of the document to download
	 * @param refresh indicates whether the refresh of the cached document shall be
	 *                enforced
	...
	public DSSDocument getDocument(final String url, final boolean refresh) {
		...
		if (refresh) {
			...
			// Check cache expiration considering nextUpdate for revocation data
			byte[] existingData = null;
			try {
				existingData = DSSUtils.toByteArray(file);
			} catch (Exception e) {
				LOG.debug("Failed to read existing cache file: {}", e.getMessage());
			}

			boolean isCacheExpired = isCacheExpired(file, existingData);
			if (isCacheExpired) {
				LOG.debug("Cache expired for url '{}'.", url);
			} else {
				LOG.info("Cached file was used for url {}.", url);
		...
		throw new DSSExternalResourceException(
				String.format("Cannot retrieve data from url [%s]. Empty content is obtained!", url));
	...
	/**
	 * Allows to add a given array of {@code byte} as a cache file representing by
	 * the {@code url}.
	 *
	 * @param urlString
	...
	public byte[] post(final String urlString, final byte[] content) throws DSSException {
		...
		if (fileExists) {
			...
			boolean isCacheExpired = isCacheExpired(file, existingData);
			if (!isCacheExpired) {
				LOG.info("Cached file was used for url {}.", urlString);
				return DSSUtils.toByteArray(file);
			} else {
				LOG.debug("Cache expired, will fetch fresh data");
			}
		} else {
	...
	private boolean isCacheExpired(File file, byte[] revocationData) {
		if (!file.exists()) {
			return true;
		}

		// Try to extract nextUpdate from revocation data if available
		Date nextUpdate = extractNextUpdate(revocationData);
		if (nextUpdate != null) {
			Date currentTime = new Date();
			if (currentTime.after(nextUpdate)) {
				LOG.debug("Cache expired based on nextUpdate field: {}", nextUpdate);
				return true;
			}
			// Cache is still valid based on nextUpdate
			return false;
		}

		// Fallback to default cache expiration time
		if (defaultCacheExpirationTime < 0) {
			return false;
		}
		long currentTime = new Date().getTime();
		if (currentTime - file.lastModified() >= defaultCacheExpirationTime) {
			LOG.debug("Cache expired based on default expiration time");
			return true;
		}
		return false;
	}
	...
	/**
	 * Extracts the nextUpdate field from revocation data (CRL or OCSP)
	 *
	 * @param revocationData the raw bytes of revocation data
	 * @return the nextUpdate Date if available, null otherwise
	 */
	private Date extractNextUpdate(byte[] revocationData) {
		if (Utils.isArrayEmpty(revocationData)) {
			return null;
		}

		try {
			// Determine if this is CRL or OCSP based on the dataLoader type
			if (dataLoader instanceof OCSPDataLoader) {
				return extractOCSPNextUpdate(revocationData);
			} else if (dataLoader instanceof CommonsDataLoader) {
				return extractCRLNextUpdate(revocationData);
			}
		} catch (Exception e) {
			LOG.debug("Failed to extract nextUpdate from revocation data: {}", e.getMessage());
		}
	...
	/**
	 * Extracts nextUpdate from OCSP response data
	 */
	private Date extractOCSPNextUpdate(byte[] ocspData) {
		try {
			OCSPResp ocspResp = new OCSPResp(ocspData);
			if (ocspResp.getResponseObject() instanceof BasicOCSPResp) {
				BasicOCSPResp basicResp = (BasicOCSPResp) ocspResp.getResponseObject();
				SingleResp[] responses = basicResp.getResponses();
				if (responses != null && responses.length > 0) {
					// Get the first response (typically there's only one)
					return responses[0].getNextUpdate();
				}
			}
		} catch (Exception e) {
	...
	/**
	 * Extracts nextUpdate from CRL data
	 */
	private Date extractCRLNextUpdate(byte[] crlData) {
		try {
			...
			LOG.debug("Failed to parse CRL: {}", e.getMessage());
		}
		return null;
	}
